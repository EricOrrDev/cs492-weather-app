...
class HourlyTemperatureGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (activeForecast != null && forecastProvider.hourlyForecasts.isNotEmpty) {
      ...
      hourlyForecasts = forecastProvider.hourlyForecasts
          .where((hourly) {
            return (hourly.startTime
                    .isAtSameMomentAs(activeForecast.startTime) ||
...
class _TemperaturePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final graphWidth =
        (size.width - (paddingHorizontal * 2)).clamp(0, double.infinity);
    final graphHeight =
        (size.height - paddingTop - paddingBottom).clamp(0, double.infinity);

    final xStep =
        forecasts.length > 1 ? graphWidth / (forecasts.length - 1) : 0.0;
    ...
    for (var i = 0; i < forecasts.length; i++) {
      ...
      final normalizedTemp =
          tempRange > 0 ? (temps[i] - minTemp) / tempRange : 0.5;
    ...
    for (var i = 0; i <= gridLines; i++) {
      ...
      canvas.drawLine(Offset(paddingHorizontal, y),
          Offset(size.width - paddingHorizontal, y), gridPaint);
    }
